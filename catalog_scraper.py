...
def scroll_page(driver, steps=8):
    """Scroll in steps so lazy-loaded product cards render."""
...
def parse_listing(html: str):
    """Pull url, name and price out of the product cards on a category page."""
    ...
    for a in soup.select('a[href^="/product/"]'):
        ...
        # Price sits in a sibling span, matched by the currency symbol
...
def main():
    ...
    try:
        ...
        input("Complete verification if required, wait for products to load, then press Enter...")
        ...
        while len(rows) < LIMIT:
            ...
            if not items:
                print("[stop] No products detected. Verification or location selection may be required.")
                print("Open the page in the browser, make sure products are visible, then press Enter.")
                input("Enter...")
                driver.refresh()
                time.sleep(2)
                continue
            ...
            for it in items:
                ...
                # Autosave so a long run is not lost on a crash
                if len(rows) % 10 == 0:
